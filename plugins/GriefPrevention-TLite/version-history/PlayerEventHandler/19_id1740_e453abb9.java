...
class PlayerEventHandler implements Listener {
    ...
    public class YourClass {
        ...
        public void onSpecialItemUse(PlayerInteractEvent interactEvent) {
            if (interactEvent.getAction() == Action.RIGHT_CLICK_BLOCK) {
                ...
                if (itemID == 19263) {
                    ...
                    if (claim != null && claim.allowAccess(player) && isAtLeast10BlocksAway(playerLocation, claim)) {
                        // Player is allowed to use the item
                        return;
                    }

                    // Player is not allowed to use the item
        ...
        // Helper method to check if the player is at least 10 blocks away from the claim's border
        private boolean isAtLeast10BlocksAway(Location playerLocation, Claim claim) {
            Cuboid claimBounds = claim.getGreaterBoundary();
            return claimBounds.isInBounds(playerLocation, 10);
        }
    }
