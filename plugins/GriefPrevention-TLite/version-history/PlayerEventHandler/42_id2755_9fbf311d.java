...
class PlayerEventHandler implements Listener {
    ...
    public void onEntropyItemUse(PlayerInteractEvent interactEvent) {
        if (interactEvent.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            final int itemID = player.getInventory().getItemInHand().getType().getId();

            if (itemID == 4363 || itemID == 4364) {
                PlayerData playerData = GriefPrevention.instance.dataStore.getPlayerData(player.getName());
                Claim claim = GriefPrevention.instance.dataStore.getClaimAt(clickedBlock.getLocation(), false, playerData.lastClaim);

                if (claim == null || (claim.allowAccess(player) != null && !claim.allowAccess(player).isEmpty())) {
                    GriefPrevention.sendMessage(player, TextMode.Err, Messages.Entropy);
                    interactEvent.setCancelled(true);

                    // Execute only if event is cancelled
                    if (interactEvent.isCancelled()) {
                        // Remove the item from the player's hand
                        final ItemStack entropyItem = player.getInventory().getItemInHand();
                        player.getInventory().setItemInHand(new ItemStack(Material.AIR));

                        // Add the item back to the player's inventory after a short delay (1 tick)
                        Bukkit.getScheduler().runTaskLater(GriefPrevention.instance, new Runnable() {
                            @Override
                            public void run() {
                                player.getInventory().addItem(entropyItem);
    ...
    // EE3 will throw a fit every time the worldTransmute event is cancelled, it's fine.
    // Block usage in wilderness.
    @EventHandler(ignoreCancelled = true, priority = EventPriority.LOWEST)
        ...
        if (interactEvent.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (itemID == 27002) {
                ...
                if (claim == null || (claim.allowAccess(player) != null && !claim.allowAccess(player).isEmpty())) {
                    ...
                    if (interactEvent.isCancelled()) {
                        // Remove the item from the player's hand
                        final ItemStack miniumStone = player.getInventory().getItemInHand();
                        player.getInventory().setItemInHand(new ItemStack(Material.AIR));
                        ...
                        Bukkit.getScheduler().runTaskLater(GriefPrevention.instance, new Runnable() {
                            @Override
                            public void run() {
                                player.getInventory().addItem(miniumStone);
                                player.updateInventory(); // Update the player's inventory to reflect changes
                            }
                        }, 1);
                    }
        ...
        if (interactEvent.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (itemID == 19263) {
                ...
                if (claim == null || (claim.allowAccess(player) != null && !claim.allowAccess(player).isEmpty())) {
                    ...
                    // Check distance to the nearest claim border
                    int distanceToBorder = GriefPrevention.instance.dataStore.getClaimWorldManager(player.getWorld()).getDistanceToNearestBorder(clickedBlock.getLocation(), claim);

                    if (distanceToBorder < 10) {
                        GriefPrevention.sendMessage(player, TextMode.Err, "You cannot use Wrath Igniter within 10 blocks of the claim border.");
                        return; // Cancel the interaction if too close to the claim border
                    }

                    // Execute only if event is still cancelled
                    if (interactEvent.isCancelled()) {
                        final ItemStack wrathIgniter = player.getInventory().getItemInHand();
                        player.getInventory().setItemInHand(new ItemStack(Material.AIR));

                        Bukkit.getScheduler().runTaskLater(GriefPrevention.instance, new Runnable() {
                            @Override
                            public void run() {
                                player.getInventory().addItem(wrathIgniter);
