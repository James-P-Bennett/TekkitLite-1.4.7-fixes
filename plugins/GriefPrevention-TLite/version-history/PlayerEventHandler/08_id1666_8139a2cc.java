			...
			if (itemID == 19263) {
				...
				if (claim != null) {
					...
					if (claimBounds.isInBounds(playerLocation, 10)) {
						// Player is within 10 blocks
						return;
					} else {
						// Player is in a claim but not within 10 blocks of the claim border
						GriefPrevention.sendMessage(player, TextMode.Err, "You can only use this item 10 blocks within your claim border.");
						interactEvent.setCancelled(true);
						return;
					}
				}
				...
				GriefPrevention.sendMessage(player, TextMode.Err, "You can only use this item within claims.");
				interactEvent.setCancelled(true);
			}
		}
	}
