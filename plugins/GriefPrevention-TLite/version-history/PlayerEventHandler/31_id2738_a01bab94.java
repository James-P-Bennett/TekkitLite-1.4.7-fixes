...
class PlayerEventHandler implements Listener {
    ...
    // Block usage in wilderness.
    // Your existing code for preventing Minium Stone usage outside of trusted claims
    @EventHandler(ignoreCancelled = true, priority = EventPriority.LOWEST)
        ...
        if (interactEvent.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (itemID == 27002) {
                ...
                if (claim == null || !claim.allowAccess(player).isEmpty()) {
                    ...
                    // Remove the Minium Stone from the player's hand temporarily
                    final ItemStack miniumStone = new ItemStack(27002, 1); // Assuming 27002 is the correct item ID
                    player.getInventory().setItemInHand(miniumStone);

                    // Schedule a task to give the Minium Stone back after a short delay
                    Bukkit.getScheduler().runTaskLater(this, new Runnable() {
                        @Override
                        public void run() {
                            player.getInventory().setItemInHand(miniumStone);
                        }
                    }, 20L); // 20L = 1 second (adjust as needed)
                }
            }
        }
    }
