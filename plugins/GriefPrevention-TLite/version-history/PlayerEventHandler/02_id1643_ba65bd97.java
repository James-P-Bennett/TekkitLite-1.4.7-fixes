...
class PlayerEventHandler implements Listener 
{
	...
	public void onCustomItemUse(PlayerInteractEvent interactEvent) {
		if (interactEvent.getAction() == Action.RIGHT_CLICK_BLOCK) {
			...
			// Check if the item in the player's hand has the ID 4363 or 4364
			...
			if (itemID == 4363 || itemID == 4364) {
				...
				if (claim == null || !claim.allowAccess(playerData)) {
					// The player is not in a claim or doesn't have access to the claim
					GriefPrevention.sendMessage(player, TextMode.Err, "You can only use this item within trusted claims.");
