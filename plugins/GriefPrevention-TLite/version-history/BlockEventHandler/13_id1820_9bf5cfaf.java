...
public class BlockEventHandler implements Listener 
{
	...
	@EventHandler(ignoreCancelled = true, priority = EventPriority.LOWEST)
	public void onStorageMonitorClick(PlayerInteractEvent interactEvent) {
		if (interactEvent.getAction() == Action.RIGHT_CLICK_BLOCK) {
			Player player = interactEvent.getPlayer();
			Block clickedBlock = interactEvent.getClickedBlock();

			// Check if the clicked block is an ME Storage Monitor
			if (clickedBlock.getType() == Material.getMaterial("blkStorageMonitor")) {
				PlayerData playerData = this.dataStore.getPlayerData(player.getName());

				// Get the claim at the clicked location
				Claim claim = this.dataStore.getClaimAt(clickedBlock.getLocation(), false, playerData.lastClaim);

				// Check if the claim is valid and the player is at least 10 blocks away from the claim's border
				if (claim != null) {
					String accessResult = claim.allowAccess(player);

					// Assuming "allowed" is the expected string indicating access is allowed
					if ("allowed".equals(accessResult)) {
						// Player is allowed to interact with the ME Storage Monitor
						return;
					}

					// Player is not allowed to interact with the ME Storage Monitor
					GriefPrevention.sendMessage(player, TextMode.Err, "You can only use ME Storage Monitors within trusted claims.");
