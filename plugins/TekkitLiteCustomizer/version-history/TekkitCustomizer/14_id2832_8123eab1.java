...
public class TekkitCustomizer extends JavaPlugin
{
	...
	//HashMap for AdjacentBlockEventHandler
	Map<String, List<MaterialInfo>> config_adjacentBanned = new HashMap<>();
	...
	{ 		
		...
		// Register for AdjacentBlockEvents
		AdjacentBlockEvents adjacentBlockEvents = new AdjacentBlockEvents(this);
	...
	private void loadConfiguration()
	{
		...
		// AdjacentBlockEvents
        // Disallow placing certain blocks adjacent to one another to prevent duping
		// Read configuration settings for AdjacentBlockEvents
		List<Map<?, ?>> adjacentBannedConfig = config.getMapList("TekkitCustomizer.Bans.AdjacentBanned");
		for (Map<?, ?> entry : adjacentBannedConfig) {
			String groupName = (String) entry.get("group");
			List<String> materialStrings = (List<String>) entry.get("materials");

			List<MaterialInfo> materialInfoList = new ArrayList<>();
			for (String materialString : materialStrings) {
				MaterialInfo materialInfo = MaterialInfo.fromString(materialString);
				if (materialInfo != null) {
					materialInfoList.add(materialInfo);
				}
			}

			config_adjacentBanned.put(groupName, materialInfoList);
		}
	...
	public Map<String, List<MaterialInfo>> getConfigAdjacentBanned() {
		return config_adjacentBanned;
	}
