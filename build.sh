...
#   MODS  COREMODS  MCPC  MFR_SRC  EE3_SRC  AE_SRC  FZ_SRC  IC2_SRC  IMMIBIS_SRC  RPCORE_SRC  BC_SRC  CC_SRC  TE_SRC  COFH_SRC  IRONCHEST_SRC  LP_SRC  AP_SRC  NC_SRC  OT_SRC  WM_SRC  WR_SRC  SC_SRC  APM_SRC  TC_SRC  NEI_SRC  ASM  JAVAC8
#
...
for j in "$MFR_SRC" "$EE3_SRC" "$AE_SRC" "$FZ_SRC" "$PCC" "$IC2_SRC" "$IMMIBIS_SRC" "$RPCORE_SRC" "$BC_SRC" "$CC_SRC" "$TE_SRC" "$COFH_SRC" "$IRONCHEST_SRC" "$LP_SRC" "$AP_SRC" "$NC_SRC" "$OT_SRC" "$WM_SRC" "$WR_SRC" "$SC_SRC" "$APM_SRC" "$TC_SRC" "$NEI_SRC" "$CCC" "$BSPKRS"; do
